...
int main() {
    ...
    props.SetProperty("dbname", "dummydb");
    props.SetProperty("status", "true");
    props.SetProperty("sleepafterload", "0");

    // workload
    const std::string& workload_name = std::string(CMAKELISTS_PATH) + "/ycsb/workloads/workload_1K.ini";
    ...
    input.close();
    // db property
    const std::string& db_property = std::string(CMAKELISTS_PATH) + "/ycsb/properties/dummydb.properties"; 
    ...
    if (!do_load && ! do_transaction) {
        std::cerr << "No operation specified, exit" << std::endl;
        exit(1);
    }
