...
int main() {
  ...
  const std::string& workload_name =
      std::string(CMAKELISTS_PATH) + "/ycsb/workloads/workload_ssd";
  ...
  input.close();
  // db property
  const std::string& db_property =
      std::string(CMAKELISTS_PATH) + "/ycsb/properties/ssd.properties";
  ...
  if (do_load) {
    const int total_ops =
        std::stoi(props[ycsbc::CoreWorkload::RECORD_COUNT_PROPERTY]);
  ...
  std::this_thread::sleep_for(std::chrono::seconds(
      std::stoi(props.GetProperty("sleepafterload", "0"))));

  std::cout << "after load, table_info_set size: " << table_info_set.size()
            << std::endl;
  ...
  if (do_transaction) {
    ...
    const int total_ops =
        std::stoi(props[ycsbc::CoreWorkload::OPERATION_COUNT_PROPERTY]);
  ...
  std::cout << "after transaction, table_info_set size: "
            << table_info_set.size() << std::endl;
...
现在还不能记录table对应的stripe_id,
只能遍历stripe_info_set,找到包含这个table的stripe,然后在stripe中把这个table标记为失效
-> O(n) 否则可以立刻去stripe_info_set中找到这个table对应的stripe,
...
在tableinfo中标记sst的失效位,然后在每次调用RemoveObsoleteFiles的时候,遍历table_info_set和stripe_info_set,判断是否要删除一个条带
-> O(n) 目前版本

还有一个思路是在每次标记失效位之后就立刻更新stripe_info_set,
然后超过阈值就立刻调用删除stripe  -> 未实现
