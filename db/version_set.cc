...
int Version::LowLevelEc(int forced, StripeRecorder& stripeRecorder)
{
  ...
  for (int level_tmp = 0; level_tmp <= config::maxlowlevel; level_tmp++)
  {
    for(size_t i=0;i<files_[level_tmp].size();i++)
    {
      ...
      if(numtoec == 4)
      {
        ...
        Log(vset_->options_->info_log, "%sWe will do ec for these files : %ld(%d->%d) %ld(%d->%d) %ld(%d->%d) %ld(%d->%d) %d files should be changed : %ld bytes ; ec : %d bytes\n",
         forced_str,
  ...
  { 
    ...
    for(int j=0;j<numtoec;j++)
    {
      ...
      std::cout << "LowLevelEc::AddTable FORCED, adding: " << filestoec[j]->number << "->" << filestoec[0]->leader_number << std::endl;
